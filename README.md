...
1)A node.js frontend server that serves views to the user and makes fetch api calls to the backend server through event listeners(js scripts inside the views).
2)A java backend server that recives HTTP requests on it's endpoints, takes care of validation, login logic, email verification and persists user information to the SQL server through JPA.
...
1) The frontend server includes several directories and files:
   ...
   - static directory that contains all static resources
     ...
     . js directory that contains all js scripts, that attach event handlers, handle http requests from the backend server, display validation errors and redirects the user if needed
        ~ All scripts attach the header, so I will not be including it in further script specification
        ~ register.js attaches event listeners, sends a fetch api request to the backend and displays validation errors.
        ~ login.js attaches event listeners on the login view and sends user information to the backend server. Upon a 200 response it saves session token and email to the session storage and redirects to index(when the session token is stored the user has succesfully logged in). Upon 403 response it adds an additional email verification token field. Upon a 400 request it displays all validation errors.
        ~ edit-profile.js attaches event listeners, sends a fetch api request to the backend server (including the session verification token), displays validation errors and updates stored in the session email if its changed.
...
2) The backend server includes two main directoris main and test. The server logic is inside the main and unit tests are inside the test
   ...
   - controllers directory incudes all controllers that handle incoming http requests
     . Register controller handles two types of requests - "POST" and "OPTIONS".
       ...
       ~ When a POST request is recieved the request body (type json sent from the frontend) it gets parsed to a local JSONObject, the input is validated and if all user information is correct a new user is created and persisted to the DB, otherwise the appropriate response is sent, including all validation errors. Email verification email is sent as well.
   ...
   - email directory includes the Email sender class, that sends account confirmation emails through jakarta mail api with google as smtp server host.
   - model directory includes two entities - Base Entity and User Entity, that are annotated with the appropriate JPA annotations
     . Base entity, that is inherited by all other entities and includes Id incrementation  logic
     . User entity, that contains all user fields, that are persisted to the DB, including a active flag, that is set to true when the email is verified and the current session validation token, that is sent to the user in their most recent login.
   - service directory includes the User service, that takes care of CRUD operations to the DB through JPA entity manager
   - utils directory includes utility class HTTPUtils, that provides additional CORS header attachment and request body parsing logic
...
In order to start my application you should: 
   ...
   2) Open the backend directory inside intelij or another enviroment and run the project from there or do "mvn clean install" and than start the jar file
